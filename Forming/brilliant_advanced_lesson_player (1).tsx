...
const MascotSVG = ({ className }) => (
  <svg viewBox="0 0 100 100" className={className} style={{ filter: 'drop-shadow(0px 10px 15px rgba(74, 222, 128, 0.4))' }}>
    <path d="M50 5 L95 50 L50 95 L5 50 Z" fill="#4ADE80" />
    <path d="M50 15 L85 50 L50 85 L15 50 Z" fill="#22C55E" />
    <rect x="38" y="38" width="24" height="24" rx="6" fill="#0A0A0A" />
    <rect x="42" y="42" width="16" height="8" rx="2" fill="#FFF" />
  </svg>
);

const SplashIconSVG = () => (
  <div className="relative w-32 h-32 flex items-center justify-center">
    <svg viewBox="0 0 100 100" className="absolute w-20 h-20 text-yellow-500 -ml-12 -mt-8 animate-[spin_4s_linear_infinite]">
      <rect x="20" y="20" width="60" height="60" rx="16" fill="currentColor" />
      <path d="M40 50 L48 58 L65 40" stroke="#000" strokeWidth="8" strokeLinecap="round" strokeLinejoin="round" fill="none" />
    </svg>
    <svg viewBox="0 0 100 100" className="absolute w-24 h-24 text-green-400 ml-8 mt-8 shadow-2xl z-10">
      <rect x="15" y="15" width="70" height="70" rx="24" fill="currentColor" />
      <rect x="40" y="40" width="20" height="20" rx="4" fill="#111" />
    </svg>
  </div>
);
...
const PodiumSVG = () => (
  <svg viewBox="0 0 200 100" className="w-64 h-32 absolute bottom-0 left-1/2 -translate-x-1/2">
    <ellipse cx="100" cy="50" rx="80" ry="25" fill="#EAB308" opacity="0.3" style={{ filter: 'blur(10px)' }} />
...
export default function AdvancedLessonPlayer() {
  ...
  const [mappingSlots, setMappingSlots] = useState([null, null, null]);
  const [eqSlots, setEqSlots] = useState([null, null, null]);
  
  // New States for Bins, Highlights, Connects, Runner
  const [binItems, setBinItems] = useState({}); // { itemId: binId }
  const [activeToken, setActiveToken] = useState(null); // for dragging/tapping
  
  const [selectedText, setSelectedText] = useState([]);
  
  const [activeConnectNode, setActiveConnectNode] = useState(null);
  const [connections, setConnections] = useState([]); // [{from, to}]
  
  const [runnerSelection, setRunnerSelection] = useState([null, null, null]);
           ...
           <div className="relative w-full h-40 flex items-center justify-between z-10 px-4">
              ...
              <div className="flex flex-col items-center z-20">
                 <div className={`w-16 h-16 border-2 flex items-center justify-center rounded-xl bg-[#0A0A0A] transition-all
                    ...
                    {runnerSelection[0] && <MascotSVG className="w-10 h-10 grayscale" />}
                 </div>
        ...
        <div className="flex items-end relative w-1/2 min-h-[80px]">
          {globalPhase === 'lesson' && (
            <div className="relative">
              <MascotSVG className="w-16 h-16 drop-shadow-[0_0_15px_rgba(34,197,94,0.3)] animate-[bounce_3s_infinite]" />
      ...
      {globalPhase === 'intro' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0A0A0A]">
          <div className="flex flex-col items-center animate-[dropIn_2.5s_ease-in-out_forwards]">
             ...
             <MascotSVG className="w-24 h-24 z-10" />
          </div>
        </div>
      )}
        ...
        {globalPhase === 'complete' && (
          <div className="flex-1 flex flex-col items-center justify-center animate-in fade-in zoom-in duration-700">
             <div className="relative w-64 h-64 flex items-center justify-center mb-8">
                ...
                <MascotSVG className="w-24 h-24 absolute top-10 animate-bounce z-10" />
                <PodiumSVG />
             </div>
